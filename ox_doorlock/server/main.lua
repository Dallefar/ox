...
local function isAuthorised(source, door, lockpick)
	...
	if player then
		if lockpick then
			return DoesPlayerHaveItem(source, lockpickItems)
		end
		...
		if door.groups then
			authorised = IsPlayerInGroup(source, door.groups) or nil
		end

		if not authorised and door.items then
			authorised = DoesPlayerHaveItem(source, door.items) or nil
		end

		if authorised ~= nil and door.passcode then
			authorised = door.passcode == lib.callback.await('ox_doorlock:inputPassCode', source)
		end
	end
	...
	if not authorised and IsPlayerAceAllowed(source, ('doorlock.%s'):format(door.name)) then
		authorised = true
	end
